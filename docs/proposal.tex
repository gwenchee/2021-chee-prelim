...
The REALM simulations proposed in Table \ref{tab:realm_simulations} could be 
...
Additive manufacturing of nuclear reactor components is a quickly 
developing field thanks to breakthroughs in additive manufacturing fabrication 
of metal components. 
...
For preliminary work, I designed the \gls{REALM} Python package that applies 
...
The motivation for \gls{REALM} is to enable reactor designers to utilize 
...
algorithm optimization methods to this reactor type, and also participate in 
the \gls{OECD} \gls{NEA}'s \gls{FHR} benchmarking exercise. 

For preliminary work, I applied \gls{REALM} to maximize $k_{eff}$ 
in the \gls{AHTR} fuel slab by varying the \gls{TRISO} particle packing fraction 
distribution. 
The \gls{TRISO} particle packing fraction distribution that produced the final 
generation's maximum $k_{eff}$ peaked at the slab's center.
...
Therefore in this chapter, I propose to further explore using REALM for 
multi-objective \gls{AHTR} optimization of arbitrary geometries and fuel distribution.