...
The \gls{REALM} optimization objective aims to maximize the slab $k_{eff}$. 
...
In a \gls{REALM} input file, the user defines hyperparameters for the genetic 
algorithm.
...
\begin{figure}[]
    ...
    \begin{subfigure}{\textwidth}
        ...
        \label{fig:pf_39}
    \end{subfigure}
    \caption{Each generation's results for \gls{REALM}'s genetic algorithm 
    optimization of the Straightened \acrfull{AHTR} Fuel Slab. The \gls{REALM} 
...
qualities, such as maximal heat transfer and minimal power peaking across the 
core. 
