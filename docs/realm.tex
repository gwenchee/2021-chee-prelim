...
\end{figure}
Therefore, \gls{REALM} will initially read and validate the JSON input 
...
In the subsequent sections, I will describe the evolutionary algorithm software 
...
For each iteration of the loop, the selection, mating, and mutation operators 
are applied to the population, after which the new individuals are evaluated, 
the constraints are applied, and the results are saved. 
...
For each evaluator, mandatory input parameters include: \texttt{input\_script}, 
\texttt{inputs} and \texttt{outputs}, and optional input parameters include: 
\texttt{output\_script}. 
The \texttt{input\_script} is a input file template for the evaluator software. 
...
The \texttt{outputs} parameter lists the output parameters that the user 
...
Second, the user can use pre-defined evaluations. 
For example, we have predefined a keff evaluation for OpenMC. 
The user may also add pre-defined evaluations to \texttt{OpenMCEvaluation} or 
...
the output parameters' names and its corresponding value as key/value pairs. 
...
\texttt{evaluators} specifications in the input file and uses the 
\textbf{\textit{Evaluator}} object to create a function that will run each 
evaluator software with the desired input parameters and return the output 
parameters calculated by the evaluator software. 
...
\textbf{\textit{Evaluator}} object and registers it as the `evaluate` tool
in the toolbox.  
...
A user should first set up the REALM \texttt{json} input file and evaluator 
...
REALM's results are managed by the \textbf{\textit{BackEnd}} class. 
\textbf{\textit{BackEnd}} saves all the results in a dictionary which is 
pickled (Python module that serializes Python objects) and saved as 
