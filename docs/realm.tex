...
public access. 
In the preliminary work, I enabled \gls{REALM} to work with open-source nuclear 
...
making them suitable to be used as \gls{REALM}'s nuclear dependency.
However, users can easily use restricted nuclear software if they so desire 
with \gls{REALM} by coupling \gls{REALM} with the restricted software on their 
local machine. 
...
In this section, I will describe the \gls{REALM} v1.0 software architecture and 
...
\begin{table}[]
    \centering
    \onehalfspacing
    \caption{Classes that make up the \gls{REALM} architecture. }
    ...
    \end{tabular}
    \end{table}
Figure \ref{fig:realm_archi} depicts the \gls{REALM} software architecture. 
\begin{figure}[]
    \centering
...
When running \gls{REALM} from the command line, there is one mandatory argument and 
...
\noindent
\texttt{python realm -i <input file name> -c <checkpoint file name>}
...
The \textbf{\textit{BackEnd}} class manages results from each \gls{REALM} simulation. 
...
\gls{REALM} is hosted on Github \cite{chee_arfcrealm_2021}. 