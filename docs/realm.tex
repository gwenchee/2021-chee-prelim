...
\begin{table}[]
    ...
    \textit{InputValidation} & The \textit{InputValidation} class contains methods 
    ...
    \hline
    \textit{Evaluation} & \gls{DEAP}'s fitness evaluator (as mentioned in Section 
    ...
    \hline 
    \textit{OpenMCEvaluation} & The \textit{OpenMCEvaluation} class contains
    ...
    \hline 
    \textit{ToolboxGenerator} & The \textit{ToolboxGenerator} class initializes
    DEAP's \textit{toolbox} and \textit{creator} modules with genetic algorithm 
    hyperparameters defined in the user input file.\\
    \hline
    \textit{Constraints} & The \textit{Constraints} class contains methods to 
    ...
    \hline 
    \textit{BackEnd} & The \textit{BackEnd} class contains methods to save 
    ...
    \hline
    \textit{Algorithm} & The \textit{Algorithm} class contains methods to 
    ...
    \textit{ToolboxGenerator}, applies constraints defined in
    \textit{Constraints}, evaluates fitness values using the evaluation 
    function produced by \textit{Evaluation}, and saves all the results 
    with \textit{BackEnd}. \\
    \hline
    \textit{Executor} & The \textit{Executor} class drives the \gls{REALM} code
    execution with the following steps: \\
    & 1) User input file validation with \textit{InputValidation}. \\
    & 2) Evaluation function generation with \textit{Evaluation}. \\
    & 3) DEAP toolbox initialization with \textit{ToolboxGenerator}. \\ 
    & 4) Constraint initialization with \textit{Constraints}. \\ 
    & 5) Genetic algorithm execution with \textit{Algorithm}. \\
    \hline
    \end{tabular}
    \end{table}
...
\begin{figure}[]
    ...
    \begin{tikzpicture}[node distance=0.4cm]
        ...
        \node (6) [b223block, right=of 2, yshift=0cm, xshift=1cm] 
        ...
        \node (7) [b223block, right=of 4, yshift=-1cm, xshift=1cm] 
